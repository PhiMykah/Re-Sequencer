...
{
    return (o3_p[ia][ipa] > 0.0 && o3_p[ib][ipb] > 0.0) ? p1p2_dist(o3_p[ia] + ipa - 4, o3_p[ib] + ipb - 4) : -1.0;
}
                    ...
                    double **xyz_pair, char **Miscs, long out_org)
{
    ...
    for (i = 1; i <= num; i++)
    {
        k = serial[i];
        p = Miscs[k];
        fprintf(fp, "%s%5ld %4s%c%3s %c%4ld%c   %8.3f%8.3f%8.3f%s\n", (p[0] == 'A') ? "ATOM  " : "HETATM", i, AtomName[k], p[1], ResName[k], ChainID[k],
