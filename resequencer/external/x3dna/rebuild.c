...
static void rebuild_cmdline(int argc, char *argv[], char *options[], char *user_input,
                            char *inpfile, char *outfile, long *neg_xdir)
{
    long i, j, nop = 4;
    if (argc < 2)
        rebuild_usage();
...
static void read_pars(char *inpfile, struct_pars *pars)
{
    ...
    pars->xdir = sum_twist < 0.0 && !num_positive_twist;
    if (ip == pars->num_bp)
        pars->parallel = 1;
}
...
int main(int argc, char *argv[])
{
    ...
    long num_bp, num_atoms, num_max_per_residue, neg_xdir = 0;
    struct_pars pars;
    time_t time0;
    time(&time0);
    remove_file(REF_FILE);
    set_my_globals(argv[0]);
    strcpy(user_input, options[2]);
    rebuild_cmdline(argc, argv, options, user_input, inpfile, outfile, &neg_xdir);
    init_pars(&pars);
    ...
    if (!strcmp(user_input, options[0])) {
        ...
        if (pars.is_single)
            atomic_pdb1(num_bp, num_atoms, num_max_per_residue, pars.is_helical,
                        pars.xdir, pars.bp_seq, pars.step_par, BDIR, outfile);
        else
        ...
        if (pars.is_single)
            ...
            atomic_base_p(pars.parallel, num_bp, num_atoms, num_max_per_residue,
                          pars.is_helical, pars.xdir, pars.bp_seq, pars.bp_par,
                          pars.step_par, BDIR, pars.pidx, outfile);
        }
