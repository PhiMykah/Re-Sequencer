...
{
    sprintf(FDIR, "%s/fiber_data/", Gvars.X3DNA_HOMEDIR);
}
...
{
    char README_FILE[BUF2K], str[BUF512];
    FILE *fp;
...
{
    long i, j;
    // Print help message if there are less than 2 arguments
    if (argc < 2)
        fiber_usage();

    // Set default args as a baseline
    ...
    for (i = 1; i < argc; i++)
    {
        // Ensure that the first character is a - sign for options
        if (*argv[i] != '-')
            break;
        // Check if any global options have been inserted for the program
        ...
        if (lux_ncmatch(argv[i], "^--?re"))
        {
            args->repeat = get_lvalue(argv[i], 1, BUF2K);
            continue;
        }
...
static char *extract_valid_sequence(char *sequence, long repeat, char *valid_bases,
                                    long *num_bp)
{
    long i, k, nb = 0;
    char *bseq, *full_seq;
    ...
    {
        nb *= repeat;
        full_seq = cvector(0, nb);
        for (i = 1; i <= repeat; i++)
            strcat(full_seq, bseq);
        free_cvector(bseq, 0, DUMMY);
        bseq = full_seq;
    }
    *num_bp = nb;
    return bseq;
...
{
    if (Gvars.mmcif)
        write_mmcif(tnum, tAtomName, tResName, tChainID, tResSeq, txyz, args->pdbfile);
    else
    {
        if (args->connect)
            atom_lkg(tnum, tAtomName, tResName, tChainID, tResSeq, txyz, args->pdbfile);
        else
                    ...
                    struct_args *args)
{
    ...
    if (is_empty_string(b_rep))
    {
        if (is_empty_string(args->sequence))
            bseq = get_sequence(Wb, &num_bp);
        else
            bseq = extract_valid_sequence(args->sequence, args->repeat, Wb, &num_bp);
    }
    else
        bseq = read_repeat(b_rep, 1, Wb, &num_bp);
    bp_seq = single2double(num_bp, bseq, Wb, Cb);
    for (i = 0; i < 4; i++)
        idx[i] = 0;
    for (i = 1; i <= num_bp; i++)
        for (j = 1; j <= 2; j++)
    ...
    for (i = 1; i <= num_bp; i++)
    {
        ...
        num1 =
            read_pdb(fname, NULL, sAtomName, sResName, sChainID, sResSeq, sxyz, NULL, 1, "*");
        for (j = 1; j <= num1; j++)
        {
            ik = tnum + j;
...
{
    char base_rpt[BUF512], fname[BUF2K];
    ...
    for (i = 1; i <= n_rep; i++)
    {
        if (lval_in_range(args->str_id, 38, 41) || lval_in_range(args->str_id, 48, 52))
        {
            dphi = (i - 1) * twist;
            dz = (i - 1) * rise;
        }
...
{
    char triplet[BUF512], fname[BUF2K];
    char *ChainID1, *ChainID2, *tChainID;
...
static void extract_sequence_to_3chains(char *sequence, long repeat, char *seq1, char *seq2,
                                        char *seq3)
{
    ...
    seqOK = extract_valid_sequence(items[1], repeat, bases, &len);
    ...
    free_cvector(seqOK, 0, DUMMY);
    strcat(bases, "0");
    seqOK = extract_valid_sequence(items[2], repeat, bases, &len);
    assert(len < BUF2K);
    strcpy(seq2, seqOK);
    if (nitem == 2)
    {
    ...
    free_cvector(seqOK, 0, DUMMY);
    seqOK = extract_valid_sequence(items[3], repeat, bases, &len);
    assert(len && len < BUF2K);
    strcpy(seq3, seqOK);
    free_cvector(seqOK, 0, DUMMY);
}
...
static void pauling_triplex(char *FDIR, struct_args *args)
{
    ...
    long n1, n2, n3;
    extract_sequence_to_3chains(args->sequence, args->repeat, seq1, seq2, seq3);
...
int fiber_entry(int argc, char *argv[])
{
    ...
    else if (lval_in_range(str_id, 44, 45))
        residue(twist, rise, "A", 0, FDIR, &args);
    else if (lval_in_range(str_id, 46, 47) || lval_in_range(str_id, 53, 55))
        residue(twist, rise, "", -1, FDIR, &args);
