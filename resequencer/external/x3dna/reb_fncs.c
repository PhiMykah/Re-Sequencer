    ...
    for (i = 1; i <= num_residue - 1; i++) {
        ...
        if (ib && ie && ChainID[ib] == ChainID[ie]) {
            ...
            if (d_o3_p < Gvars.misc_pars.o3p_dist) {
                ...
                lsort(connect[ib][7], connect[ib], idx);
                if (connect[ie][7] + 1 > 6) {
                    fprintf(stderr, "atom <%ld: [%s]> has over 6 bonds\n", ie, AtomName[ie]);
                    continue;
                } else {
                ...
                lsort(connect[ie][7], connect[ie], idx);
            } else
              ...
              double **xyz, char *outfile)
{
    ...
    if (Gvars.mmcif) {
        write_mmcif(num, AtomName, ResName, ChainID, ResSeq, xyz, outfile);
        return;
    }
                 ...
                 long xdir, char **bp_seq, double **step_par, char *BDIR, char *outfile)
{
    ...
    close_file(fp);
    atom_lkg(tnum, tAtomName, tResName, tChainID, tResSeq, txyz, outfile);
                   ...
                   double **step_par, char *BDIR, long *pidx, char *outfile)
{
    ...
    while (fgets(str, sizeof str, fp) != NULL) {
        ...
        if (ik == 3) {
            if (parallel) {
                fprintf(stderr,
                        "Only 3 coordinates provided. Copied used for the" " other 3\n");
                ppos[i][4] = ppos[i][1];
                ppos[i][5] = ppos[i][2];
                ppos[i][6] = ppos[i][3];
            } else {
