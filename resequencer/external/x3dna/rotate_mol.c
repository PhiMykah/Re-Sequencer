...
static void flip_x_or_y(double *x, double *y, double **rotmat)
{
    ...
    if (((fabs(x[2]) > fabs(x[1])) && x[2] < 0.0) || ((fabs(y[2]) > fabs(y[1])) && y[2] > 0.0)) {
        rotx(180.0, rot180);
    ...
    if (((fabs(y[1]) > fabs(y[2])) && y[1] > 0.0) || ((fabs(x[1]) > fabs(x[2])) && x[1] < 0.0)) {
        roty(180.0, rot180);
        multi_matrix(tmpmtx, 3, 3, rot180, 3, 3, rotmat);
        if (Gvars.VERBOSE)
...
static void pmi_view(double **rotmat, double **tmprot, long c1, long c2, long c3)
{
    long i;
    for (i = 1; i <= 3; i++) {
        tmprot[i][1] = rotmat[i][c1];
        tmprot[i][2] = rotmat[i][c2];
        tmprot[i][3] = rotmat[i][c3];
    }
}
